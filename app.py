...
<div class="container">
  ...
  <div class="progress-card" id="progress-card">
    <p style="text-align:center; margin-bottom:8px;">در حال پردازش...</p>
  ...
  <p class="supported">پشتیبانی از: YouTube • Instagram • TikTok • Twitter/X • Facebook و بیشتر</p>
</div>
<script>
...
function checkProgress() {
  const interval = setInterval(async () => {
    ...
    if (data.progress) {
      ...
      document.getElementById('progress-text').textContent = data.status || 'در حال پردازش...';
    }
    if (data.done) {
...
def do_download(dl_id, url, dl_type, quality):
    try:
        ...
        if not video_id:
            downloads[dl_id]['error'] = 'لینک یوتیوب معتبر نیست'
            return
        ...
        downloads[dl_id]['status'] = 'در حال پردازش...'
        ...
        if dl_type == 'audio':
            ...
            if data.get('status') != 'ok':
                ...
                for _ in range(10):
                    ...
                    if data.get('status') == 'ok':
                        break
            if data.get('status') != 'ok':
                downloads[dl_id]['error'] = 'خطا در دریافت MP3: ' + str(data.get('msg', 'نامشخص'))
                return

            download_url = data.get('link')
            filename = data.get('title', 'audio') + '.mp3'

        else:
            # MP4 از youtube-to-mp4-mp3
            headers['x-rapidapi-host'] = 'youtube-to-mp4-mp3.p.rapidapi.com'
            resp = requests.get(
                'https://youtube-to-mp4-mp3.p.rapidapi.com/mp4',
                params={'videoid': video_id, 'quality': quality + 'p'},
                headers=headers,
                timeout=60
            )
            data = resp.json()
            ...
            if isinstance(data, dict):
                ...
                # پیدا کردن کیفیت مناسب
                target = quality + 'p'
                item = None
                for d in data:
                    if str(d.get('quality', '')) == target or str(d.get('qualityLabel', '')) == target:
                        item = d
                        break
                if not item:
                    item = data[0]
            ...
            if not download_url:
                downloads[dl_id]['error'] = 'لینک دانلود پیدا نشد: ' + str(data)[:200]
                return

        downloads[dl_id]['progress'] = 70
        downloads[dl_id]['status'] = 'در حال دانلود فایل...'
        ...
        if not safe_name:
            safe_name = 'download.mp4' if dl_type == 'video' else 'download.mp3'
        ...
        downloads[dl_id]['file'] = filepath
        downloads[dl_id]['filename'] = safe_name
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'api_key': bool(RAPIDAPI_KEY)})
